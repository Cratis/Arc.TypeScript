...
- Generated server artifact metadata and complete .NET proxy parity. The source analyzer emits a bounded client model, but standard-mode injection still needs explicit tokens and recorded validator rules need the programmatic bridge; identity-only models, derived types and some .NET template options are not yet emitted.
