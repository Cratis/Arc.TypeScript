...
describe('when converting SQL column values', () => {
    it('should round-trip concrete concepts and provider GUID types', () => {
        ...
        conceptCodec(TaskName, 'string', DrizzleDialect.SQLite).fromDriver('name').value.should.equal('name');
        conceptCodec(TaskNumber, 'number', DrizzleDialect.SQLite).fromDriver(42).value.should.equal(42);
    });
    it('should reject a mismatched kind and support indexed MySQL string concepts', () => {
        (() => conceptCodec(TaskName, 'number' as 'string', DrizzleDialect.SQLite)).should.throw('does not match number');
        conceptCodec(TaskName, 'string', DrizzleDialect.MySQL, 120).sqlType.should.equal('varchar(120)');
        (() => conceptCodec(TaskName, 'string', DrizzleDialect.MySQL, 0)).should.throw('varcharLength');
    });
