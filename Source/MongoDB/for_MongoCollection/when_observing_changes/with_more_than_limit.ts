...
describe('when observing changes with more than the configured limit', given(a_replica_set, context => {
    ...
    beforeEach(async () => {
        ...
        const database = { command: sinon.stub().resolves({ setName: 'rs0' }) } as unknown as Db;
        const collection = new MongoCollection(native, database, TaskRecord, context.context('a'), false, 1);
