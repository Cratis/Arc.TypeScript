...
export class MongoDocumentCodec<T extends object> {
    ...
    constructor(private readonly type: new () => T, private readonly ignoreConventions = false,
        private readonly namingPolicy: MongoNamingPolicy = defaultMongoNamingPolicy) {
    ...
    serialize(value: T): Document {
        ...
        const runtime = value.constructor as WireField['type'];
        const document = this.encodeObject(fieldsFor(runtime).length ? runtime : this.type, value, true);
        if (!Object.hasOwn(document, '_id') || document._id == null)
            throw new Error(`MongoDB model ${this.type.name} requires a key value`);
        return document;
    }
    ...
    private encodeObject(type: WireField['type'], value: object, root = false): Document {
        ...
        for (const field of fields) {
            ...
            if (content !== undefined) result[field.name === this.#key && root ? '_id' :
                this.namingPolicy.propertyName(field.name)] = this.encodeValue(field.type, content, field.element);
        }
    ...
    private encodeValue(type: WireField['type'], value: unknown, element?: WireField['type']): unknown {
        ...
        if (value instanceof Guid) return new Binary(
            Buffer.from((value as Guid).toString().replaceAll('-', ''), 'hex'), Binary.SUBTYPE_UUID);
        ...
        if (fieldsFor(type).length) {
            ...
            return this.encodeObject(DerivedType.get(runtime as new () => object) ? runtime : type, value as object);
        }
    ...
    private decodeObject(type: WireField['type'], document: Document, root = false): object {
        ...
        const actual = identifier == null || !derivatives.length ? type :
            derivatives.find((candidate: new () => object) => {
                const declared = DerivedType.get(candidate);
                return typeof declared === 'string' && typeof identifier === 'string' &&
                    Guid.isGuid(declared) && Guid.isGuid(identifier)
                    ? declared.toLowerCase() === identifier.toLowerCase() : declared === identifier;
            });
        ...
        for (const field of fieldsFor(actual)) {
            const name = root && field.name === this.#key ? '_id' : this.namingPolicy.propertyName(field.name);
    ...
    private decodeValue(type: WireField['type'], value: unknown, element?: WireField['type']): unknown {
        ...
        if (type === Number) {
            if (value instanceof Decimal128 || value instanceof Long) {
                ...
                if (!Number.isFinite(number) || value instanceof Long && !Number.isSafeInteger(number) ||
                    value instanceof Decimal128 && number.toString() !== value.toString())
                    throw new RangeError('MongoDB number cannot be represented exactly as a JavaScript number');
                return number;
            }
            if (typeof value !== 'number' || !Number.isFinite(value)) throw new TypeError('MongoDB number must be finite');
            return value;
        }
        if (type === Date || type === String || type === Boolean || type === ObjectId || type === Binary) return value;
