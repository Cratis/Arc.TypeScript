...
export const arc0005 = ESLintUtils.RuleCreator.withoutDocs({
    meta: { type: 'problem', docs: { description: 'Consume values produced by provide()' }, messages: { unused: 'Command {{name}} provides a value but handle() has no binding for it.' }, schema: [] },
    defaultOptions: [],
    create(context) {
        return { ClassDeclaration(node) {
            ...
            if (!provider?.value.body || handler?.value.params.length || (!handler && inheritedMethod(context, node, 'handle'))) return;
            if (provider.value.body.body.some(statement => statement.type === AST_NODE_TYPES.ReturnStatement && statement.argument &&
                !(statement.argument.type === AST_NODE_TYPES.Identifier && statement.argument.name === 'undefined') &&
                !(statement.argument.type === AST_NODE_TYPES.UnaryExpression && statement.argument.operator === 'void') &&
                ...
                context.report({ node: provider.key, messageId: 'unused', data: { name: node.id?.name ?? '<anonymous>' } });
            }
        } };
    }
});
