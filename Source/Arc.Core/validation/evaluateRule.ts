...
export async function evaluateRule(rule: Rule, value: unknown, model: unknown, signal: AbortSignal): Promise<boolean> {
    ...
    switch (rule.kind) {
        ...
        case 'greaterThan': return value == null || (compare(value, first) ?? -Infinity) > 0;
        case 'greaterThanOrEqual': return value == null || (compare(value, first) ?? -Infinity) >= 0;
        case 'lessThan': return value == null || (compare(value, first) ?? Infinity) < 0;
        case 'lessThanOrEqual': return value == null || (compare(value, first) ?? Infinity) <= 0;
        case 'inclusiveBetween': return value == null || (compare(value, first) ?? -Infinity) >= 0 &&
            (compare(value, second) ?? Infinity) <= 0;
        case 'exclusiveBetween': return value == null || (compare(value, first) ?? -Infinity) > 0 &&
            (compare(value, second) ?? Infinity) < 0;
