...
export function reflectedParameters(type: ClassType | object, member: string, count: number, offset = 0): readonly ServiceIdentifier<unknown>[] {
    ...
    if (tokens.length !== count || tokens.some(token => typeof token !== 'function' || token === Object || token === Function || token === Array))
