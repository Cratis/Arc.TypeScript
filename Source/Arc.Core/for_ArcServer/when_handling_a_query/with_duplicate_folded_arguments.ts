...
describe('when handling a query with duplicate folded arguments', given(a_server_with_a_list_query, context => {
    let response: Response | null;
    let body: { validationResults: { reason: string }[] };

    beforeEach(async () => {
        response = await context.server.handle(new Request('http://localhost/api/tasks/list?limit=1&LIMIT=2'));
