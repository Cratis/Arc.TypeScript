...
describe('when selecting a command client response with bare operation arrays', () => {
    let error: unknown;
    beforeEach(() => {
        ...
        const method = file.statements.filter(ts.isClassDeclaration).find(owner => owner.name!.text === 'BareOperations')!.members.find(ts.isMethodDeclaration)!;
    ...
    it('should reject bare operation arrays at their declarations', () => {
        (error as Error).message.should.contain('Use CommandOperations');
    });
});
