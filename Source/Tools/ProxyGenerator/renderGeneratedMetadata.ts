...
export function metadataCollector(program: ts.Program, output: string): {
    ...
    return {
        ...
        render(project, artifacts) {
            ...
            const functions = ['abortSignal', 'commandContext', 'commandReadModel', 'optionalService', 'provided'].filter(name =>
...
export function renderGeneratedMetadata(project: string, artifacts: string, output: string, program = sourceProgram(project)): string {
    const checker = program.getTypeChecker();
    ...
    return collector.render(project, artifacts);
}
