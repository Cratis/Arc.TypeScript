...
export function renderCommand(operation: SourceOperation, path: string, destinations: ReadonlyMap<string, string>, route: string,
    ...
    const validation = renderRecordedRules(name, 'CommandValidator', `I${name}`, rules, diagnostic, operation.fields.map(field => field.name));
