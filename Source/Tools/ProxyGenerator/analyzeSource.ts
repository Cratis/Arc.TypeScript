...
export function analyzeSource(project: string, artifacts: string, rootNamespace = '', generatedMetadata = false): SourceAnalysis {
    ...
    const module = join(root, 'generatedMetadata.ts');
    const hasMetadata = generatedMetadata || existsSync(module) &&
        readFileSync(module, 'utf8').includes('// @generated by Cratis. Regenerate with arc-proxygenerator --metadata. Hash: ');
            ...
            if (isModel) for (const member of declaration.members) {
                ...
                for (const parameter of member.parameters) {
                    ...
                    if (!binding) {
                        ...
                        if (!serviceBinding && !(inferredService && (hasMetadata || config.options.experimentalDecorators === true)))
