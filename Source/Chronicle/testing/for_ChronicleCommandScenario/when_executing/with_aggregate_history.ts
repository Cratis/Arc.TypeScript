...
describe('when executing with aggregate history in memory', given(a_reduced_command, context => {
    let scenario: ChronicleCommandScenario<InstanceType<typeof context.aggregate>>;
    let result: Awaited<ReturnType<typeof scenario.execute>>;
    beforeEach(async () => {
        scenario = context.create(context.aggregate);
        scenario.given.forEventSource('source-a').events(new context.added(2));
        result = await scenario.execute({ id: 'source-a' });
    });
    afterEach(async () => { await scenario.dispose(); });
    it('should point to a kernel-backed scenario instead of pretending to replay', () => {
