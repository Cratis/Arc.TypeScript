...
@eventType() class Unappended { }

should();
describe('when a nested array contains a decorated event as a Chronicle client response', given(a_command_with_typed_ports, context => {
    let success: boolean;
    let errors: string[];
    beforeEach(async () => {
        const { server } = context.setup(async () => [accepted()], undefined, [Placed], [[new Unappended()]]);
        try {
    ...
    it('should reject the event even when it is not registered with the store', () => {
