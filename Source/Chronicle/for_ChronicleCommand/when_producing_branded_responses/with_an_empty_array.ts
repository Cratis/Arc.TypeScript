...
should();
describe('when an empty array is produced as a Chronicle client response', given(a_command_with_typed_ports, context => {
    let success: boolean;
    let errors: string[];
    beforeEach(async () => {
        const { server } = context.setup(async () => [accepted()], undefined, [Placed], []);
        try {
    ...
    it('should reject it because the Chronicle response handler claims an empty array as an append', () => {
