...
export class ArcApplicationBuilder {
    ...
    add(...types: ClassType[]): this {
        for (const type of types) {
            ...
            if (!metadata.command && !metadata.readModel && !metadata.lifetime && !metadata.validatorTarget && !metadata.responseValueHandler && !metadata.queryRenderer && !metadata.readModelInterceptor && !isIdentityDetailsProvider(type)) {
