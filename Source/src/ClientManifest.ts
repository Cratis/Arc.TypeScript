...
export function validateClientManifest(value: unknown): ClientManifest {
    if (!plain(value) || value.product !== '@cratis/arc.server' || value.version !== 1 || !Array.isArray(value.operations) || (value.operations.length === 0 || value.operations.length > 1024) ||
    ...
    return { product: '@cratis/arc.server', version: 1, operations: operations.sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0) };
}
...
export function exportClientManifest(server: ArcServer): ClientManifest {
    ...
    return validateClientManifest({ product: '@cratis/arc.server', version: 1, operations });
}
