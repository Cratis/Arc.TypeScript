...
`provide()` is optional. It runs after validation and before `handle()` on the same command instance; its one preparation value becomes the **first** `handle` argument, before injected services. Returning `rejected(...)` or `denied(...)` stops execution. This is intentionally narrower than .NET's assignable-type matching and flattening of multiple provided values: to hand `handle()` several values, return one object that carries them. `provide()` takes no parameters. When it needs a service, resolve the service from the execution scope with `await currentServices().resolve(CreditBureau)`, and read the request's cancellation signal from `currentContext()?.signal`; both are exported by `@cratis/arc.core`. `build()` cannot preflight a service resolved this way, so a missing registration surfaces when the command runs. Put other inputs on command fields, not in `handle()` parameters.
