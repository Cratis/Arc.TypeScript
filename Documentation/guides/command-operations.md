...
This excerpt is adapted from the [operation specification](../../Source/Arc.Core/for_ArcServer/given/an_operation_command.ts). It records calls in memory to make the order visible; replace that recorder with a real service before using it for business effects.
...
A nested Arc command inside an operation is unsupported, even if you ignore its failed result. Return another declaration instead. The [command outcomes guide](command-outcomes.md) covers ordinary responses and rejection without operations.
