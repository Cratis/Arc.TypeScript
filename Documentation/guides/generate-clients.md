...
The CLI accepts `--segments-to-skip <nonnegative integer>`, `--api-prefix <prefix>`, `--skip-command-name-in-route`, and `--skip-query-name-in-route`; they use the same route functions as the server. `--use-proxy-file-suffix` is optional. Local imports are **extensionless by default**, matching .NET output and the published `@cratis/arc` client in Vite/Bundler frontends. Use `--js-import-specifiers` when your generated TypeScript must also run as native Node ESM after compilation. This option changes only local imports, not filenames or package imports. Generated files carry the .NET-style source/time/hash header and a do-not-edit block. Repeated runs keep byte-identical content and timestamps; changed generated files are replaced and stale owned files removed. Handwritten files are never overwritten or deleted. The output root must already exist and be a real directory. The Tasks sample also has `yarn workspace @cratis/arc.core.sample.tasks generate-proxies`, which writes to its ignored `dist/proxies` folder after `yarn build`.

The source analyzer currently handles standard and legacy decorators recognized through their imported symbols, decorated fields, `ConceptAs<T>`, primitives, Fundamentals dates and GUIDs, arrays, string/number enums, string-literal unions, `Promise<T>`, `ObservableSource<T>`, `AsyncIterable<T>` and model types. Query arguments require explicit `@query(argument(...))` bindings for runtime use. Unsupported result types fail with a file/line diagnostic rather than falling back to `any`. It reads literal, unconditional `ruleFor` chains in `@validator(Target)` constructors and emits client-safe rules for commands (including direct concept fields) and explicit query `argumentsModel` targets. Conditions, predicates and rules that need runtime evaluation stay on the server and produce source diagnostics; the server still checks every request. Derived classes with Fundamentals `@derivedType('id')` and their declared base classes are emitted when found under the artifacts root. The generator does **not** yet generate server metadata, infer erased service tokens, discover identity-only types from `identityDetails`, or reproduce every .NET output template. Standard-mode `@inject()` and bare `@query()` still need explicit tokens/descriptors. The programmatic `renderSource` also accepts a `recordedRules` override. Do not treat this bounded output as complete .NET proxy parity.
