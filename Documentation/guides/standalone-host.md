...
With a path base, pass the same `pathBase` as the fourth `attachNodeWebSockets` argument. Supply a trusted native resolver as its third argument for host-authenticated upgrades; ordinary HTTP authentication configuration does not automatically protect raw Node upgrades. `createArcNodeHandler` does not manage listener errors, SSE shutdown, or connection draining: the caller owns those on the Node server.
