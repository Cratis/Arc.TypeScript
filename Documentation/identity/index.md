...
The handler chain uses the first authenticated or failed result; an anonymous result lets the next handler try. `jwtBearer()` rejects an invalid bearer token and verifies the signature, issuer, audience, expiration, issued-at and subject. It uses `jose`'s remote JWKS and accepts a nonnegative `clockSkewSeconds` (default zero). It requires an HTTPS JWKS and an explicit asymmetric algorithm list. Keep tokens out of URLs. `microsoftIdentityPlatform()` is opt-in: partial headers are anonymous, malformed encoded principals fail authentication. Neither handler turns the `.cratis-identity` cookie into a credential.
