...
- **Integrations** give commands and queries somewhere to read and write. They are separate packages that depend on the core, never the other way around. `@cratis/arc.mongodb` serves tenant-scoped MongoDB collections, `@cratis/arc.drizzle` serves tenant-scoped SQL reads, and `@cratis/arc.chronicle`, experimental and not private since v0.12.0, appends events returned from a command; its live-kernel suite covers Express, Fastify, and Hono, but it does not provide .NET transaction, aggregate, or reactor-command parity.
