...
- **Integrations** give commands and queries somewhere to read and write. They are separate packages that depend on the core, never the other way around. `@cratis/arc.mongodb` reads query results from MongoDB. `@cratis/arc.chronicle` is an experimental package that appends events returned from a command; its live-kernel suite covers Express, Fastify, and Hono, but it does not provide .NET transaction or reactor-command parity.
