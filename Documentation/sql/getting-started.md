...
For a first SQL-backed Arc query, install `drizzle-orm` and `better-sqlite3` alongside `@cratis/arc.core` and `@cratis/arc.drizzle` from this source workspace. Packages are not published to npm. Run your schema migration before serving requests; `addDrizzle` never creates tables.
...
Registration in your own builder follows the same shape:
