...
description: Announce committed writes to tenant-scoped Drizzle read models and stream current, paged SQL results.
---
...
Install `rxjs` alongside `@cratis/arc.drizzle`: it is a peer dependency for SQL observation. Enable `DrizzleObservation.InProcess` explicitly. Without it, starting an observation fails; ordinary read APIs and validated `notifyChanged` calls still work.
...
import { ArcApplication, command, inject, key, query, queryOptions, readModel, service, type QueryOptions } from '@cratis/arc.core';
...
export class TaskListing {
    ...
    static all(records: DrizzleReadModels<TaskRecord>, options: QueryOptions) {
        return records.observePage(undefined, options);
    }
}
