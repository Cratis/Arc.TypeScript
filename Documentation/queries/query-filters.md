...
at the validator stage. A pre-aborted direct query resolves or invokes no declared authorization policy. Arc checks cancellation before each named policy resolution and invocation and after each awaited resolution, as well as before each filter, validator dependency or callback, performer dependency, `perform()`, and renderer/interceptor resolution and invocation. If cancellation occurs while one of these stages is pending, Arc waits for it to settle, fails the request without starting the next policy or stage or the snapshot/observable query producer, and releases its operation scope. Already-running callbacks are not interrupted.

The admission-filter specs exercise in-process observable hub admission and direct SSE for health. The conformance suite pairs GET, `QUERY`, observable snapshots, and direct SSE filter denial and allow against .NET on Express, Fastify, and Hono. It also checks direct WebSocket denial and allow, and that the host keeps serving direct SSE/WebSocket subscriptions after an early disconnect, on each TypeScript adapter; the .NET HTTP fixture has no direct WebSocket upgrade. TypeScript fixture traces check authorization before ordinary filters before validation and verify that denial does not construct validator/performer dependencies or start the observable producer. See [Query pipeline](query-pipeline.md) for the complete order and [Authorizing commands and queries](../authorizing-commands-and-queries.md) for declared authorization.
