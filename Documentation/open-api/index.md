...
Arc serves an OpenAPI 3.1 document at `GET /openapi.json`. It describes registered commands as `POST` and queries as `GET`, including observable queries. Set `openApiVersion` when constructing the server or builder to advertise your application's version; the default remains `0.1.0`.
...
Protected operations advertise HTTP bearer security when `jwtBearer()` is registered as a default or named authentication handler; anonymous operations do not. Arc cannot infer the protocol of a custom handler, so it does not advertise one as bearer. Native-principal authentication is supplied by the host and has no bearer scheme in this document.
