...
`@cratis/eslint-plugin-arc-core` checks model-bound artifacts before you build. Enabled rules report as errors. Both presets include Chronicle append checks; `recommended-type-checked` also includes the Guid response check. `query-argument-name` is opt-in. Rules that match a .NET analyzer keep its `ARC` or `ARCCHR` code. Setup is in [Code analysis](../code-analysis/index.md).
...
| [`arcchr0007`](../code-analysis/ARCCHR0007.md) | Direct default-log appends inside a command's `handle()` |
| [`arcchr0010`](../code-analysis/ARCCHR0010.md) | A keyless command returning a raw `Guid.parse(...)` beside an event (type-checked preset) |
