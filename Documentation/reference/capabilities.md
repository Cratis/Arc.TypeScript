...
| Named policies and schemes | Bounded | Function or scoped class policies; named Arc handlers selected per operation. Schemes on observable queries fail at build. See [Authorization policies and schemes](../core/authorization.md). | `Source/Core/for_ArcServer/when_authorizing_named_schemes.ts`, `.../when_authenticating_a_handler_scheme.ts`, `.../when_rejecting_observable_scheme_configuration.ts` |
...
| [Chronicle](../chronicle/index.md) | Experimental | `addChronicle` appends returned model-bound events through a response value handler, resolves Chronicle read models by command key, and batches nested returned events. SDK 6.5.1 loads in native Node ESM. No aggregate or reactor-command parity. | `Source/Chronicle/for_ChronicleResponseHandler`, `.../for_ChronicleUnitOfWork`, `.../for_ChronicleCommandScenario`, `bash Source/Chronicle/run-integration.sh` (live kernel, Docker) |
...
- **Invalid GUID query arguments.** .NET 22.22.0 binds `Guid.Empty` and returns 200; TypeScript rejects the malformed UUID with 400.
...
A paired suite, `yarn test:conformance`, sends the same 48 checks to a .NET host built on the published `Cratis.Arc` 22.22.0 package and to Arc for TypeScript mounted in Express. It covers command execution, validation-only requests, authorization before validation including validation-only denial, business-rule and malformed-input rejection, tuple response validation, GET and `QUERY` binding, paging, sorting, exception redaction, unsupported methods, correlation IDs, model-bound commands and validation, acronym naming, enum and named-float output, an overridden query path, and a conventional GUID query route with valid and invalid inputs. It pins the deliberate differences above, and two observed differences that are not choices of Arc for TypeScript: the .NET host does not apply `sortBy` on GET, while the structured `QUERY` request sorts identically on both; and Express answers an unknown path with its own HTML 404 without an Arc correlation header. It needs the .NET 10 SDK and is not part of `yarn ci`. It is a bounded check of those routes, not a claim of full parity.
