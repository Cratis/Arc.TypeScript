...
| Several return values and response handlers | Supported, bounded | `tuple(...)` flattens branded return groups and `response(...)` branches. At most one unhandled value is the client response; a registered scoped `CommandResponseValueHandler` consumes each other value (first match). Ordinary arrays are not flattened; there is no general OneOf/Result union classification or automatic returned Chronicle event handling. |
...
| Command context and keys | Supported, bounded | `CommandContext` carries the command, a captured key, case-insensitive provider values and request identity. `@key()` or `getKey()` marks the default key; scoped application key rules take precedence. `signal()` and `context()` explicitly bind method parameters; `provided(Type)` matches selected `provide()` values by runtime type. Without those descriptors `provide()` still passes one value first. Handler read-model ownership arbitration/injection is not implemented. |
