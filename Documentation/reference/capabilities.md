...
| Several return values and response handlers | Supported, bounded | `tuple(...)` flattens branded return groups and `response(...)` branches. At most one unhandled value is the client response; a registered scoped `CommandResponseValueHandler` processes each other value with every matching handler in deterministic name order. Ordinary arrays are not flattened; the private Chronicle integration handles registered event arrays as a batch. No general OneOf/Result union classification. |
| Command operations and effects | Supported, bounded | `CommandOperation` and `CommandOperations` are server-only declarations. Arc preflights dependencies, executes sequentially, and attempts reverse compensation after an explicitly uncommitted failure. Commit participants must report `NoCommit`/`NotCommitted`/`Committed`/`Unknown`/`Mixed`; uncertain outcomes fail closed. No distributed transaction, durable recovery or process-crash guarantee. See [Declare command operations](../guides/command-operations.md). |
| Command context and keys | Supported, bounded | `CommandContext` carries the command, a captured key, case-insensitive provider values and request identity. `@key()` or `getKey()` marks the default key; scoped application key rules take precedence. `abortSignal()` and `commandContext()` explicitly bind method parameters; `provided(Type)` matches selected `provide()` values by runtime type. Without those descriptors `provide()` still passes one value first. Handler read-model ownership arbitration/injection is not implemented. |
...
| Validation rules shared with the client | Partial, source-analyzed | The source analyzer emits literal, unconditional client-safe `ruleFor` rules from `@validator(Target)` command and explicit query-arguments validators; server-only statements and rules produce diagnostics. `renderSource` also accepts a programmatic rule override. No runtime execution of application validators; the server still validates every request. |
...
| Capability | Status | Notes |
| --- | --- | --- |
| TypeScript proxy generation | Supported, bounded source preview | `analyzeSource` and `generateFromSource` read a TypeScript 6 program with decorated commands, queries, fields, concepts, enums and observable returns without importing application startup. The CLI accepts `--project`, `--artifacts`, `--output`, `--root-namespace`, route options, optional `.proxy.ts` suffix, interface model emission, and index/deletion switches; it emits hooks, namespace folders and marked, hash-checked owned barrels, with stale-file cleanup that preserves hand-written barrels and their directory/package re-exports. Tasks-generated proxies compile against published `@cratis/arc` and `@cratis/arc.react` 22.19.1 with `skipLibCheck: false` and run commands, parameterized/paged/sorted queries and hub observable updates on live model-bound Express/Fastify/Hono hosts. It extracts the literal client-safe subset of `@validator(Target)` constructor rules and emits classes marked `@derivedType` along with their base models. It uses extensionless imports by default for Vite/Bundler, with `--js-import-specifiers` for native Node ESM. It does not yet discover identity-only types, support duplicate model class names across namespaces, or emit a generated server metadata module; full .NET parity remains unverified. The low-level `exportClientManifest` path remains available: it writes a version 1 JSON manifest from registered operations that declare an explicit `clientOutput` shape. `@cratis/arc.proxygenerator` renders it with `renderClientManifest` or writes it with `generateClient`; its CLI reads only that JSON and takes an absolute manifest path and an existing absolute output directory. Flat DTOs, basic and optional inputs, command responses, and DTO or array queries are tested with `@cratis/arc` 22.19.1, `@cratis/fundamentals` 7.19.3, and `rxjs` 7.8.2 against live Express, Fastify, and Hono hosts. Consumers must compile in strict `Bundler` mode; `skipLibCheck: false` works, and `NodeNext` consumer compilation is not supported because of the published declarations. Set the server origin on each proxy with `setOrigin`. Observable definitions with explicit `clientOutput` generate `ObservableQueryFor` proxies with their exact query name; generated proxies are compiled against the installed client and exercised over direct SSE on all three adapters. Not generated: Zod defaults, transforms, and refinements, nullable command fields, scalar query results, nested DTOs, and React hooks. Model-bound `@command()` and `@readModel()` artifacts declare no `clientOutput`, so the low-level JSON path rejects an application that registers them; use the source analyzer for them. No automatic discovery in the low-level JSON path, no npm publication, and not parity with Arc's .NET proxy generator. See [Generate command and query clients](../guides/generate-clients.md). |
...
| Command `key()` annotation | Supported, bounded | Marks the command field used as the default key on `CommandContext`; the optional Chronicle integration also uses it as the default event source ID. No automatic handler read-model lookup. |
...
| Build-time diagnostics | Supported, bounded | `add()` rejects undecorated classes; `build()` rejects authorization, path, injection, and query decorators on members with no compiled effect. The source analyzer fails unsupported client types with file/line locations and rejects unbound query parameters; it reports server-only validator statements. No Roslyn-style analyzer or generated binding metadata yet. |
...
| [Chronicle](../guides/chronicle.md) | Experimental, kernel-checked subset | Private `@cratis/arc.chronicle`: `addChronicle` registers discovered SDK artifacts and appends returned model-bound events through a response handler. 6.5.0 loads in native Node ESM. An opt-in suite checks append, readback, tenant isolation, before-first conflict, and a projected read-model query over real HTTP adapters. No aggregate, reactor-command, or nested unit-of-work parity. |
| Transactions and units of work | Not implemented | Chronicle sends one returned batch through one SDK `appendMany`, but cannot atomically include nested commands, direct appends, or other effects. MongoDB does not open a transaction. |
...
- **Preparation and binding.** By default a model-bound `provide()` passes one preparation value as the first `handle` argument. For multiple or interspersed values, use `tuple(...)` and `@inject(provided(Type), ...)` to match by declared runtime type; .NET also infers assignable types without explicit markers. TypeScript's explicit-mode queries require ordered descriptors with named arguments; compiler-generated binding metadata is not available yet.
...
- **Compilation and hosting.** Fundamentals 7.19.6 resolves in NodeNext without a local declaration shim. The Node host defaults to loopback instead of exposing a listener on every interface. File discovery imports a dedicated artifacts folder, not the bootstrap folder; moving a folder changes its derived namespace and route.
...
The shared Arc pages show a TypeScript tab beside C#, Kotlin, and Java. Its snippets live in this repository's `Documentation/client-snippets/`, and `yarn docs:snippets` compiles each one against this repository's packages with strict settings and standard decorators. Six tabs still state that TypeScript does not support their *full* workflows yet: model-bound, command-key read-model injection and the Chronicle command testing helpers those shared examples depend on. The separate experimental returned-event path is documented in the Chronicle guide. A compiled snippet shows that the API exists with that shape; it is not a parity claim for the page around it.

## How parity is checked

A paired suite (`yarn test:conformance`) sends the same 43 checks to a .NET host built on the published `Cratis.Arc` 22.22.0 package and to Arc for TypeScript mounted in Express. It covers command execution, validation-only requests, authorization before validation, business-rule and malformed-input rejection, tuple response validation, GET and `QUERY` binding, paging, sorting, exception redaction, unsupported methods, correlation IDs, a model-bound command, an overridden query path, and a conventional GUID query route with valid and invalid inputs. It pins the validator's custom state, warning threshold, and differences above. It also pins two observed differences that are not choices of Arc for TypeScript: the .NET host does not apply `sortBy` on GET, while the structured `QUERY` request sorts identically on both; and Express answers an unknown path with its own HTML 404 without an Arc correlation header. It is a bounded check of those routes, not a claim of full parity.
