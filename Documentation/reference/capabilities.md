...
| [Chronicle](../chronicle/index.md) | Experimental | Not private since v0.12.0. `withChronicle` (deprecated `addChronicle`) appends returned model-bound events through a response value handler, resolves Chronicle read models by command key, and batches nested returned events. SDK 6.5.1 loads in native Node ESM. No aggregate or reactor-command parity. | `Source/Chronicle/for_ChronicleResponseHandler`, `.../for_ChronicleUnitOfWork`, `.../for_ChronicleCommandScenario`, `bash Source/Chronicle/run-integration.sh` (live kernel, Docker) |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| Express 5, Fastify 5, and Hono 4 adapters | Supported | `cratisArc` accepts an `ArcServer` or built `ArcApplication`: Express middleware plus listener attach; Fastify plugin registers HTTP and WS in either shared-plugin order; Hono sub-app plus Node serving helper. The host keeps listener ownership; old `mount*` forms remain deprecated. See [Host adapters](../hosts/index.md). | `Source/Express/for_mountExpress`, `Source/Fastify/for_mountFastify`, `Source/Hono/for_mountHono`, `ContractTests/Client/observable-origin.test.mjs` |
| Node configuration and Fetch dispatch | Bounded | Node's application builder binds optional `appsettings.json` and `Cratis__...` environment keys for supported serializable fields, with code overrides; `app.fetch(request)` answers 404 on non-Arc paths and `app.handle(request)` falls through. Neutral-platform bundle currently fails on Node-only async context, filesystem, and transport imports; Bun, Deno, Cloudflare and Next.js deployment remain unverified. | `Source/Core/configuration/for_loadConfiguration`, `Source/Core/ArcApplication.ts` |
| Private Cratis composition | Experimental | Local `@cratis/cratis` composes Arc and Chronicle without installing an authentication handler. It is not published or verified against a live kernel in this change. | `Source/Cratis` |
