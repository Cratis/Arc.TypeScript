...
| [Chronicle](../chronicle/index.md) | Experimental | Not private since v0.12.0. `withChronicle` (deprecated `addChronicle`) appends returned model-bound events through a response value handler, resolves Chronicle read models by command key, and batches nested returned events. SDK 6.7.0 loads in native Node ESM. Keyed aggregates and returned reactor commands are experimental; full .NET parity is unverified. | `Source/Chronicle/for_ChronicleResponseHandler`, `.../for_ChronicleUnitOfWork`, `.../for_ChronicleCommandScenario`, `bash Source/Chronicle/run-integration.sh` (live kernel, Docker) |
