...
| Build-time diagnostics | Supported, bounded | `add()` rejects undecorated classes; `build()` rejects authorization, path, injection, and query decorators on members with no compiled effect. The source analyzer fails unsupported client types with file/line locations and rejects unbound query parameters; source rule rendering reports server-only rules. No Roslyn-style analyzer or generated binding metadata yet. |
