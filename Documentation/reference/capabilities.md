...
| Capability | Status | Notes |
| --- | --- | --- |
| Express 5, Fastify 5, and Hono 4 adapters | Supported | WebSocket mounting differs: Express raw upgrades bypass middleware but support an async trusted `native` resolver; Fastify uses `@fastify/websocket` routes and hooks; Hono uses `@hono/node-ws` routes and middleware. Trusted native `secure`/`authority` supports TLS-terminating proxies; `allowedOrigins` defaults to same-origin or accepts an explicit list/predicate. See [Host Arc](../guides/host-integration.md#mount-observable-websockets-on-nodejs). |
