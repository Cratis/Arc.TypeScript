...
Evidence paths are relative to the repository root. Spec folders follow `for_<Subject>/when_<action>`.
...
| Several return values and response value handlers | Bounded | `tuple(...)` flattens branded groups; at most one unhandled value becomes the response, and scoped `CommandResponseValueHandler`s process every other value in deterministic name order. Ordinary arrays are not flattened. No general OneOf or Result union classification. | `Source/Core/results/for_tuple`, `Source/Core/for_ArcServer/when_processing_command_response`, `Source/Core/for_ArcApplicationBuilder/when_registering_a_response_handler` |
...
| Command read models by key | Bounded | `commandReadModel(Type)` on `handle()` or `provide()` loads by command key through a registered resolver; MongoDB and Chronicle provide resolvers. Equal ownership claims fail. Not available in validators or for Drizzle models. | `Source/Chronicle/for_ChronicleReadModelForCommandResolver`, `Source/MongoDB/for_MongoCollection` |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| Model-bound queries | Bounded | `@readModel()` with static `@query(...)` methods and complete ordered `argument`, `service`, and `queryOptions` descriptors; a bare `@query()` infers only class-valued services under legacy emitted metadata. Identity includes namespace, read model, and method. | `Source/Core/queries/modelBound/for_compileQueries`, `Source/Core/for_ArcApplicationBuilder/when_serving_model_bound_http`, `Samples/Tasks/Features/Tasks/Listing/for_TaskItem` |
| GET and HTTP `QUERY` | Supported | `QUERY` answers with `Cache-Control: no-store` and can be turned off with `enableQueryMethod: false`. | `Source/Core/for_ArcServer/when_handling_a_query`, `ContractTests/Http/conformance.test.mjs` |
...
| Paging and sorting | Supported | Arrays are sorted and paged in memory; a provider returns `queryPage(items, totalItems, sorting?)`. GET requires `pageSize` of at least 1. See [Paging and sorting](../queries/model-bound/paging.md). | `Source/Core/for_ArcServer/when_sorting_a_query`, `Source/Core/queries/for_queryRendering`, `Samples/Tasks/Features/Tasks/Listing/for_TaskItem/when_performing/with_sorting_and_paging.ts` |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| Command and query validators | Bounded | `@validator(Target)` classes extending `CommandValidator` or `QueryValidator` with constructor-authored `ruleFor` rules; query validators target an explicit `argumentsModel`. One validator per exact target. | `Source/Core/validation/for_validator`, `.../for_ModelGraphValidator`, `.../for_RuleBuilder`, `Samples/Tasks/Features/Tasks/Registration/for_RegisterTask/when_validating` |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| Declared authorization | Supported | `@authorize`, `@roles`, `@allowAnonymous`, or `authorization`; roles in one declaration are OR, stacked declarations AND; contradictions fail at startup. See [Authorizing commands and queries](../authorizing-commands-and-queries.md). | `Source/Core/for_ArcServer/when_handling_a_protected_command`, `.../when_handling_a_protected_query`, `Source/Core/for_ArcApplicationBuilder/when_authorizing_a_model_bound_command` |
...
| [Tenant resolution](../tenancy/index.md) | Bounded | Legacy header, authoritative `resolveTenant`, and ordered header, query, claim, fixed, and strict subdomain sources with `required` and membership checks. | `Source/Core/for_ArcServer/when_configuring_tenancy`, `.../when_resolving_tenants`, `.../when_checking_tenant_membership`, `.../when_resolving_tenant_claims` |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| TypeScript proxy generation | Bounded | `arc-proxygenerator` reads decorated source and emits commands, queries, observable queries, models, hooks, derived types, and client-safe rules for the published `@cratis/arc` 22.19.1 client in strict Bundler mode. No identity-only types, duplicate model names across namespaces, server metadata, or full .NET template parity. See [Proxy generation](../proxy-generation/index.md). | `Source/Tools/ProxyGenerator/for_analyzeSource`, `.../for_renderSource`, `.../for_SourceTypeResolver`, `yarn test:client-generation` (`ContractTests/Client/source-generation.test.mjs`) |
...
| OpenAPI | Supported | `/openapi.json` is OpenAPI 3.1 with input schemas, not result schemas; `info.version` is a fixed `0.1.0`. See [OpenAPI](../open-api/index.md). | `Source/Core/for_ArcServer/when_describing_observable_query_in_openapi.ts` |
...
| Build-time checks | Bounded | `add()` rejects undecorated classes; `build()` rejects decorators with no effect, missing services, cycles, and captive lifetimes. No generated binding metadata. | `Source/Core/for_ArcApplicationBuilder/when_building`, `yarn test:decorator-types` |
...
| [Chronicle](../chronicle/index.md) | Experimental | Not private since v0.12.0. `addChronicle` appends returned model-bound events through a response value handler, resolves Chronicle read models by command key, and batches nested returned events. SDK 6.5.1 loads in native Node ESM. No aggregate or reactor-command parity. | `Source/Chronicle/for_ChronicleResponseHandler`, `.../for_ChronicleUnitOfWork`, `.../for_ChronicleCommandScenario`, `bash Source/Chronicle/run-integration.sh` (live kernel, Docker) |
| Transactions and units of work | Experimental, returned-event batch only | Chronicle stages returned events from nested commands in one tenant, correlation, and event store, and sends one `appendMany` after the outer command succeeds. Not a command-operation commit participant; no cross-sequence transaction. | `Source/Chronicle/for_ChronicleUnitOfWork` |
...
| Capability | Status | TypeScript contract | Evidence |
| --- | --- | --- | --- |
| Express 5, Fastify 5, and Hono 4 adapters | Supported | Mount functions accept an `ArcServer` or built `ArcApplication`; the host keeps listener ownership; WebSocket mounting is per framework. See [Host adapters](../hosts/index.md). | `Source/Express/for_mountExpress`, `Source/Fastify/for_mountFastify`, `Source/Hono/for_mountHono`, `ContractTests/Client/observable-origin.test.mjs` |
| Cancellation on client disconnect | Supported for Express, Fastify, and Node | Hono passes the signal of the request it received. | `Source/Express/for_mountExpress`, `Source/Fastify/for_mountFastify`, `Source/Core/http/for_createArcNodeHandler` |
...
- **Preparation and binding.** A model-bound `provide()` passes one value as the first `handle()` argument; several values need `tuple(...)` and `provided(Type)` markers, where .NET infers assignable types. Standard-mode queries need ordered descriptors.
- **Compiler metadata.** Standard decorators need explicit tokens; legacy decorators infer only class-valued dependencies from `design:paramtypes`. Observable queries declare `{ observable: true }` instead of inferring it from a return type.
...
A paired suite, `yarn test:conformance`, sends the same 58 checks to a .NET host built on the published `Cratis.Arc` 22.23.0 package and to Arc for TypeScript mounted in Express. It covers command execution, validation-only requests, authorization before validation including validation-only denial, business-rule and malformed-input rejection, tuple response validation, GET and `QUERY` binding, paging, sorting, exception redaction, unsupported methods, correlation IDs, model-bound commands and validation, acronym naming, enum and named-float output, an overridden query path, and a conventional GUID query route with valid and invalid inputs. It also checks named-policy allow and deny, numeric concept GET arguments, validation severity, and observable snapshots with current (200) and pending (202) values. It pins the deliberate differences above, and three observed differences that are not choices of Arc for TypeScript:

- The .NET host ignores `sortBy` and `sortDirection` on GET ([Cratis/Arc#2758](https://github.com/Cratis/Arc/issues/2758)), while the structured `QUERY` request sorts identically on both.
