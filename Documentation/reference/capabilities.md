...
| Several return values and response handlers | Supported, bounded | `tuple(...)` flattens branded return groups and `response(...)` branches. At most one unhandled value is the client response; a registered scoped `CommandResponseValueHandler` processes each other value with every matching handler in deterministic name order. Ordinary arrays are not flattened; there is no general OneOf/Result union classification or automatic returned Chronicle event handling. |
...
| Command `key()` annotation | Supported, bounded | Marks the command field used as the default key on `CommandContext`; it does not change routing, binding, or persistence. No automatic handler read-model lookup. |
...
| Build-time diagnostics | Supported, bounded | `add()` rejects undecorated classes; `build()` rejects authorization, path, injection, and query decorators on members with no compiled effect. The source analyzer fails unsupported client types with file/line locations and rejects unbound query parameters; it reports server-only validator statements. The separate ESLint 10 `@cratis/eslint-plugin-arc-core` package checks decorated server artifacts, including type-checked bindings; see [code analysis](../code-analysis/index.md). No generated binding metadata yet. |
...
The shared Arc pages show a TypeScript tab beside C#, Kotlin, and Java. Its snippets live in this repository's `Documentation/client-snippets/`, and `yarn docs:snippets` compiles each one against this repository's packages with strict settings and standard decorators. Six tabs state that TypeScript does not support the workflow yet: resolving a read model by command key into `provide()`, `handle()`, or a validator, and the Chronicle integration's event appends and test seeding. A compiled snippet shows that the API exists with that shape; it is not a parity claim for the page around it.
