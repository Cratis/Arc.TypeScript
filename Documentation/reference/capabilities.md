...
| Several return values | Supported, bounded | `tuple(...)` returns an array of response values. No type-directed selection of event versus response, and no automatic conversion of union variants. |
...
| Validation rules shared with the client | Partial, programmatic only | `renderSource` accepts recorded portable rules and emits a `CommandValidator`/`QueryValidator`; unsupported rules produce diagnostics and remain server-only. Extraction from `@validator(Target)` declarations is not wired yet. The server still validates every request. |
...
| Build-time diagnostics | Supported, bounded | The source analyzer fails unsupported types with file/line locations and rejects unbound query parameters; source rule rendering reports server-only rules. No generated server binding metadata yet. |
...
- **Enum fields.** TypeScript enum objects are not `@field` constructors; use a scalar `@field(String)` or `@field(Number)` alongside `@enumeration(Enum)`. .NET reflects enum field types directly.
- **Compilation and hosting.** The Tasks sample uses Bundler module resolution because the currently installed Fundamentals 7.19.3 declaration barrel cannot type-check as a NodeNext consumer. The Node host defaults to loopback instead of exposing a listener on every interface. File discovery imports a dedicated artifacts folder, not the bootstrap folder; moving a folder changes its derived namespace and route.
