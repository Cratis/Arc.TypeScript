...
| Command read models by key | Bounded | `commandReadModel(Type)` on `handle()` or `provide()` loads by command key through a registered resolver; MongoDB and Chronicle provide resolvers. Equal ownership claims fail. Chronicle additionally supports `readModelForValidation(Type, { optional: true })` in model-bound command validators; Drizzle has no command read models. | `Source/Chronicle/for_ChronicleReadModelForCommandResolver`, `Source/MongoDB/for_MongoCollection` |
...
| OpenAPI | Supported | `/openapi.json` is OpenAPI 3.1 with input schemas, result envelopes, generated typed result schemas when metadata is registered, query options and bearer security declarations for configured authentication handlers. `info.version` can be set with `openApiVersion` (default `0.1.0`). See [OpenAPI](../open-api/index.md). | `Source/Core/for_ArcServer/when_describing_observable_query_in_openapi.ts`, `.../when_serving_openapi`, `Source/Core/openApi/for_renderOpenApi`, `ContractTests/Client/source-generation.test.mjs` (JSDoc summary over HTTP) |
...
| [Chronicle](../chronicle/index.md) | Experimental | Not private since v0.12.0. `withChronicle` (deprecated `addChronicle`) appends returned model-bound events through a response value handler, resolves Chronicle read models by command key, and batches nested returned events. SDK 6.6.0 loads in native Node ESM. Keyed aggregates and returned reactor commands are experimental; full .NET parity is unverified. | `Source/Chronicle/for_ChronicleResponseHandler`, `.../for_ChronicleUnitOfWork`, `.../for_ChronicleCommandScenario`, `bash Source/Chronicle/run-integration.sh` (live kernel, Docker) |
