...
| Command read models by key | Bounded | `commandReadModel(Type)` on `handle()` or `provide()` loads by command key through a registered resolver; MongoDB, Chronicle, and Drizzle provide resolvers. Equal ownership claims fail. Drizzle requires a single declared table primary key; compound keys fail at registration. Chronicle additionally supports `readModelForValidation(Type, { optional: true })` in model-bound command validators. | `Source/Chronicle/for_ChronicleReadModelForCommandResolver`, `Source/MongoDB/for_MongoCollection`, `Source/Drizzle/for_DrizzleReadModelForCommandResolver` |
...
| [SQL with Drizzle](../sql/index.md) | Bounded | Tenant-scoped Drizzle handles, column codecs, and SQL count, sort, and page for model-bound queries. SQLite and PostgreSQL tested with real databases; MySQL not run against a live server. Command read models load by the single table primary key in the tenant scope; compound keys fail at registration. No observation, migrations, change tracking, or transactions. See [how it is checked](#sql-checks). | `Source/Drizzle/for_DrizzleReadModelForCommandResolver`, `Source/Drizzle/for_DrizzleReadModels`, `.../for_ColumnCodec`, `.../for_DrizzleModelCodec`, `.../for_withDrizzle`, `bash Source/Drizzle/run-integration.sh` (live PostgreSQL 16, Docker) |
