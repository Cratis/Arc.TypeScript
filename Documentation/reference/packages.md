...
| `@cratis/arc.chronicle` | `Source/Chronicle` | Experimental: `withChronicle`, deprecated `addChronicle`,  `eventForEventSourceId`, `eventSourceIdResponse`, `eventsWithConcurrencyScopes`, routing decorators, `notAudited`, `ChronicleReadModels`; `@cratis/arc.chronicle/testing` for `ChronicleCommandScenario` | `@cratis/arc.core`, `@cratis/arc.testing`, `@cratis/chronicle` `^6.5.1`, `@cratis/fundamentals`, `zod` |

| `@cratis/cratis` | `Source/Cratis` | Experimental, private composition: `CratisApplication.createBuilder`, `addCratis`; re-exports Arc, Chronicle and testing (`./testing`); no implicit authentication handler | Arc core, Arc Chronicle, Arc testing, Chronicle SDK, Fundamentals, `zod` |

`@cratis/cratis` is a local private preview, **not** an available npm package. Do not deploy it as a supported Chronicle integration until the SDK and live kernel checks pass. Unlike C# `AddCratis`, the TS composition does not install Microsoft identity automatically: explicitly choose an authentication handler (such as `microsoftIdentityPlatform()`) or your own trusted host principal. It composes the client, not the event-store engine.
