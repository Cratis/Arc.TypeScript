...
Every package in this repository is at version 0.34.0, the version of the source preview. **None is published to npm.** They ship ES modules only. Clone this repository, run `yarn install` and `yarn build`, and then use the packages in one of two ways:
...
| `@cratis/arc.drizzle` | `Source/Drizzle` | `withDrizzle`, `drizzleReadModel`, `drizzleDatabase`, `DrizzleReadModels`, column codecs, experimental in-process observation | `@cratis/arc.core`, `@cratis/fundamentals`, `drizzle-orm` `^0.45.0`, `rxjs` `^7.8.2` (required even without observation) |
