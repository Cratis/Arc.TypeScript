...
The conformance suite pairs command execute and `/validate` filter denial and allow against .NET through Express, Fastify, and Hono. TypeScript fixture traces verify authorization filters precede ordinary filters and validation, and denied requests construct no command validator or execute the handler. Malformed input remains an intentional divergence: TypeScript can deny raw input before reporting malformed input, while .NET rejects it first.
