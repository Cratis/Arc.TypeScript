...
- Cancellation stops pipeline stages that have not started: filters, validators, dependency resolution, `provide()` and `handle()` invocation, query `perform()` and `render()` invocation, and later response handlers. It does not interrupt work already running or replace the result of a command handler or execution runner that has completed. The command reports the actual success or failure, recovery, and operation outcomes. After a persistence provider acknowledges an inline business commit, cancellation cannot undo it; an integration must call `acknowledgeCommandCommit(context)` only after an authoritative acknowledgment (Chronicle does this after the append). Cancellation may stop waiting for Chronicle observer completion when a completion timeout is configured, but the acknowledged append still returns success with its response; the read model might not yet be updated. A genuine observer failure still fails the command.
