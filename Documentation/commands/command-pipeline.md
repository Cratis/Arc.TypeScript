...
- Cancellation stops work before an irreversible effect. After a persistence provider acknowledges an inline business commit, an aborted request cannot undo it: the command keeps its successful response rather than reporting a fictitious cancellation failure. An integration with such an inline effect must call `acknowledgeCommandCommit(context)` only after an authoritative successful acknowledgment (Chronicle does this after the append). A runner that settles successfully after cancellation is changed to a cancellation failure only when the operation recovery summary confirms `NoCommit` or `NotCommitted`. Without that evidence, Arc preserves the runner's result; an absent recovery summary does **not** prove no inline effect occurred. An actual downstream error, such as observer completion failing after persistence, still fails the command.
