...
For every scope, `scope.identity` and the `execution` argument passed to scoped factories use the creation-time context: the context fields are frozen at scope creation, and plain principals (including roles, claims, and extra fields) are detached and frozen. Class-instance principals cannot be snapshotted without losing getters, methods, or private fields; ordinary requests keep the original principal object in that frozen context, but cannot borrow the scope with `runInScope`.
